...
public class DataUtil {
	 ...
	 * @return
	 */
	public static String getKalWithUnit(int step,Context context) {
		StringBuffer sb = new StringBuffer();
		double value  = step * 18.842;
		if (value < 1000*0.01) {
			sb.append(format(value));
			sb.append(context.getString(R.string.cal));
		}else{
			sb.append(format(value/1000));
			sb.append(context.getString(R.string.kcal));
		}
		return sb.toString();
	}
	public static float getKalValue(int step) {
		return (float) ((step / 1000f) * 18.842);
	...
	/**
	 * 根据步数得到距离 并且格式化 不足100米显示米 反之公里
	 * 
	 * @param step
	 * @return
	 */
	public static String getDistanceWithUnit(int step,Context context) {
		StringBuffer sb = new StringBuffer();
		double value = step*1.0*0.416;//得到米
		if (value < 1000*0.01) {
			sb.append(format(value));
			sb.append(context.getString(R.string.mi));
		}else{
			sb.append(format1(value/1000));
	...
	public static String getAvgSpeed(int step, int sportTime,Context context) {
		StringBuffer sb =new StringBuffer();
		if (sportTime <=0) {
			sb.append("0.00");
		}else{
		...
		sb.append(context.getString(R.string.km_per_hour));
		return sb.toString();
	}
