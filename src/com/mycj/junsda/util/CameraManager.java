...
public class CameraManager {
	...
	public void openCamera(CameraOpenReady callback,int i) {
		// Log.v(TAG, "<-- 开启相机 -->");
		try {
			// int size = Camera.getNumberOfCameras();
			// //Log.v(TAG, "<-- 相机个数:" + size + "-->");
...
//	}
	...
	private class SavePictureTask extends AsyncTask<Bitmap, Void, Boolean> {
		...
		protected Boolean doInBackground(Bitmap... params) {
			...
			try {
				if (params[0] != null) {
					FileUtil.saveBitmap(params[0], context);
					return true;
				}
			} catch (Exception e) {
				// Log.v(TAG, "<-- 异步存储异常...  -->");
