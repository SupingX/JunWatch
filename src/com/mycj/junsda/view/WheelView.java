...
public class WheelView extends View {
	...
	/** 当前值与标签文本颜色 */
	private static final int VALUE_TEXT_COLOR = Color.parseColor("#EAAB44");
	

	/** 项目文本颜色 */
	private static final int ITEMS_TEXT_COLOR = Color.parseColor("#D8D6D6");
	...
	private GradientDrawable centerShadow;
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			...
			itemsPaint.setTypeface(BaseApp.TYPEFACE_NUM);
		}
		...
		if (valuePaint == null) {
			...
			itemsPaint.setTypeface(BaseApp.TYPEFACE_JIAN);
		...
		if (centerShadow == null) {
			centerShadow = new GradientDrawable(Orientation.BOTTOM_TOP,
					SHADOWS_COLORS);
		}
...
//		bottomShadow.draw(canvas);
	...
	private void drawValue(Canvas canvas) {
		...
		valuePaint.setTypeface(BaseApp.TYPEFACE_NUM);
		...
		if (labelLayout != null) {
			canvas.save();
			canvas.translate(getWidth()/2 + LABEL_OFFSET, bounds.top);
	...
	private void drawCenterRect(Canvas canvas) {
		...
		centerDrawable.setBounds(0, center - offset, getWidth(), center
				+ offset);
//		centerDrawable.setBounds(0, center , getWidth(), center
//				);
